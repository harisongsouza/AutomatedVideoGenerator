...
INTRO_VIDEO_PATH = 'C:/Users/souza/Downloads/VideoCreator/assets/videos/camadas/video_intro.mp4'
...
""" def recortar_clip(input_path, output_path_corte, duracao_alvo_segundos):
    ...
    # Verificar a duração original para fornecer um aviso se o corte for maior que o vídeo
    # Não impede a operação, pois o FFmpeg lidará com isso (usará a duração total se -t for maior)
    duracao_original = get_video_duration(input_path)
    if duracao_original is not None and duracao_alvo_segundos > duracao_original:
        print(f"Aviso [recortar_clip]: Duração alvo ({duracao_alvo_segundos:.2f}s) é maior que a duração original do clipe ({duracao_original:.2f}s). O clipe inteiro será usado.")

    cmd_cut = [
        'ffmpeg',
        '-i', input_path,
        '-t', str(duracao_alvo_segundos), # Define a duração do output
        '-c', 'copy',                   # Copia os codecs (rápido, mas corte em keyframes)
        '-y', output_path_corte
    ]

    try:
        print(f"Recortando {input_path} para {duracao_alvo_segundos:.2f}s -> {output_path_corte}...")
        # print(f"Comando FFmpeg [recortar_clip]: {' '.join(cmd_cut)}")
        ...
        print("Output (stdout):", e.stdout if e.stdout else "N/A")
        print("Erro (stderr):", e.stderr if e.stderr else "N/A")
        if os.path.exists(output_path_corte): # Tenta remover arquivo parcial/problemático
            try:
...
def main():
    ...
    for intervalo_camada in intervalos_camadas:
        if intervalo_camada["nome"] == "intervalo_introducao":
            video_duracao = get_video_duration(INTRO_VIDEO_PATH)
            ...
            clip_padronizado = padronizar_clip(
                ...
                output_path="C:/Users/souza/Downloads/VideoCreator/assets/videos/camadas/editadas/intro.mp4", 
    ...
    for i, camada in enumerate(camadas_tempos):
        ...
        clip_padronizado = padronizar_clip(
            input_path=f"C:/Users/souza/Downloads/VideoCreator/assets/videos/camadas/{camada_formatada}.mp4", 
        ...
        #camadas_lista.append(camada_obj)
        clips.append(camada_obj)

        
    #TOPICOS
    for i, topico in enumerate(topicos_tempos):
        path_video_editado = f"C:/Users/souza/Downloads/VideoCreator/assets/videos/topicos/{i}.mp4"
        #path_video_editado = f"C:/Users/souza/Downloads/VideoCreator/{i}.mp4"
    ...
    # --- RENDER ---
    with open("C:/Users/souza/Downloads/VideoCreator/data/final_list_render.json", "w", encoding="utf-8") as f:
        json.dump(clips, f, ensure_ascii=False, indent=2)
              
    ordenar_json_por_inicio("C:/Users/souza/Downloads/VideoCreator/data/final_list_render.json", "C:/Users/souza/Downloads/VideoCreator/data/final_list_render.json")

    lista_com_lacunas_preenchidas = preencher_lacunas_temporais("C:/Users/souza/Downloads/VideoCreator/data/final_list_render.json")
        
    with open("C:/Users/souza/Downloads/VideoCreator/data/dados_video_corrigido.json", "w", encoding="utf-8") as f_out:
        json.dump(lista_com_lacunas_preenchidas, f_out, indent=2, ensure_ascii=False)
    
    print("\nResultado salvo em 'dados_video_corrigido.json'")
    
    with open("C:/Users/souza/Downloads/VideoCreator/data/dados_video_corrigido.json", 'r', encoding="utf-8") as f:
        intervalos_final_list = json.load(f)
                            
    concatenar_clips_com_narracao(intervalos_final_list, AUDIO_FINAL_FILE_PATH, "C:/Users/souza/Downloads/VideoCreator/end_video.mp4")
