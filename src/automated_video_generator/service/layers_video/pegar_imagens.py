...
def executar_script_powershell(caminho_script, argumento=None):
    ...
    try:
        ...
        if caminho_script == "C:/Users/souza/Downloads/VideoCreator/utils/download_images_from_links_file.ps1":
            if path_line:
                try:
                    output_dict = json.loads(path_line)
                    if 'Path' in output_dict:
                        caminho_imagem = output_dict['Path']
                        print(f"✅ Caminho encontrado: {caminho_imagem}")
...
def verificar_arquivo_e_string(nome_arquivo, string_procurada, timeout_segundos=60):
            ...
            try:
                with open(caminho_arquivo, 'r') as arquivo:
                    ...
                    if string_procurada in conteudo:
                        print(f"Arquivo '{nome_arquivo}' foi criado e contém a string '{string_procurada}'.")
                        return True
                    else:
                        print(f"Arquivo '{nome_arquivo}' foi criado, mas não contém a string '{string_procurada}'.")
                        return False
            except Exception as e:
                print(f"Erro ao ler o arquivo '{nome_arquivo}': {e}")
                return False
        else:
            print(f"Aguardando a criação do arquivo '{nome_arquivo}' na pasta Downloads...")
            time.sleep(5)  # Espera por 5 segundos antes de verificar novamente

    print(f"Tempo limite ({timeout_segundos} segundos) excedido. Arquivo '{nome_arquivo}' não encontrado.")
    return False
       
             
def main():
    # Caminhos dos scripts
    script1 = r"C:/Users/souza/Downloads/VideoCreator/utils/duckduckgo.ps1"
    script2 = r"C:/Users/souza/Downloads/VideoCreator/utils/download_images_from_links_file.ps1"
        
    # Caminhos dos arquivos
    entrada_path = "C:/Users/souza/Downloads/VideoCreator/data/termos_chaves_para_buscar_imagem.json"

    # Carregar os intervalos entre tópicos
    with open(entrada_path, "r", encoding="utf-8") as f:
        termos_chaves = json.load(f)
    
    pasta_para_manipular = r"C:/Users/souza/Downloads/VideoCreator/assets/imagens"
    ...
    for topico_intervalo in termos_chaves:
        for images in topico_intervalo["imagens"]:
            ...
            if resultado:
                ...
                images["path"] = path_file
                #tempo_espera = random.randint(5, 15)
                #print(f"Esperando {tempo_espera} segundos antes da próxima chamada...")
                #time.sleep(tempo_espera)
            else:
                print("ERRO AO PEGAR IMAGEM. Tentando novamente em 1 min ...")
                time.sleep(60)
                deletar_arquivo_downloads("urls.txt")
                executar_script_powershell(script1, frase_de_busca)
                resultado = verificar_arquivo_e_string(nome_do_arquivo, string_para_buscar)
                ...
                if resultado:
                    ...
                    print(f"NÃO CONSEGIU BAIXAR IMAGEM '{frase_de_busca}' DO DUCKDUCKGO.")
                    path_file = "C:/Users/souza/Downloads/VideoCreator/assets/imagens/imagens_camadas/fundo_preto.jpg"
                    images["path"] = path_file
    
    saida_path = "C:/Users/souza/Downloads/VideoCreator/data/add_img_path_img_interv.json"
    # Salva em arquivo
    with open(saida_path, "w", encoding="utf-8") as f:
        json.dump(termos_chaves, f, ensure_ascii=False, indent=2)
   
        
    saida_path_2 = "C:/Users/souza/Downloads/VideoCreator/data/add_img_path_img_interv_SHORTS.json"
    ci = []
