...
def adicionar_camadas_ao_video_base(
    ...
    for i, el in enumerate(camadas):
        ...
        scaled_vid = f"[v_scaled_{i}]"
        time_shifted = f"[v_shifted_{i}]"
        final_vid = f"[el{i}]"
        output_vid = f"[out{i}]"

        # Escala e ajusta tempo
        filter_lines.append(
            f"[{input_index}:v]scale={largura}:{altura},format=yuv420p{scaled_vid}"
        )
        filter_lines.append(
            f"{scaled_vid}setpts=PTS-STARTPTS+{inicio:.3f}/TB{time_shifted}"
        )

        # Fade out (opcional)
        actual_fade = min(fade_out_duration, duracao)
        if actual_fade > 0.001:
            fade_start = fim - actual_fade
            filter_lines.append(
                f"{time_shifted}fade=type=out:start_time={fade_start:.3f}:duration={actual_fade:.3f}{final_vid}"
            )
        else:
            filter_lines.append(f"{time_shifted}null{final_vid}")

        # Overlay com tempo habilitado
        filter_lines.append(
            f"{current_overlay_input}{final_vid}overlay=x={x}:y={y}:enable='between(t,{inicio:.3f},{fim:.3f})'{output_vid}"
        )
...
if __name__ == "__main__":
    ...
    for i, topico in enumerate(topicos_tempos):
        ...
        adicionar_texto_e_imagem_sem_imagens_sobreposta(
            input_video=midia_path[0],
            output_video=path_video_editado,
            duracao_total=intervalo_duracao,
            texto=topico["word"].title(),
            texto_y=200,
            imagem_largura=200,
            imagem_altura=200,
            distancia_texto_imagem=20
    ...
    adicionar_camadas_ao_video_base(
        video_base_path="C:/Users/souza/Downloads/VideoCreator/video_final_com_mais_camadas.mp4",
        camadas=clips,
        output_path="video_final_com_mais_topicos.mp4",
