...
def adicionar_camadas_ao_video_base(
    ...
    """
    Adiciona camadas ao vídeo base com controle de qualidade via CRF e preset.

    Parâmetros padrão (recomendado):
        crf=18         -> Quase lossless, ótima qualidade
        preset='veryfast' -> Rápido, menor uso de memória

    Para máxima qualidade (lossless), você pode usar:
        crf=0
        preset='slow'
    """
    ...
    for camada in camadas:
        inputs.append(f"-stream_loop -1 -i {camada['arquivo']}")
    ...
    for i, camada in enumerate(camadas):
        input_index = i + 1
        inicio = float(camada["inicio"])
        fim = float(camada["fim"])
        intervalo_duracao = fim - inicio
        ...
        largura = camada.get("largura", resolucao.split("x")[0])
        altura = camada.get("altura", resolucao.split("x")[1])
        x = camada.get("x", 0)
        y = camada.get("y", 0)
    ...
    print("Comando FFmpeg (adicionar_camadas_ao_video_base):")
    print(" ".join(cmd))
    try:
        subprocess.run(cmd, check=True, capture_output=True, text=True)
        print("Camadas adicionadas com sucesso!")
    except subprocess.CalledProcessError as e:
        print("Erro ao adicionar camadas ao vídeo:")
        print("Comando:", e.cmd)
        print("Output:", e.stdout)
        print("Error:", e.stderr)
    

def aguardar_arquivo(path, timeout=30):
    for _ in range(timeout):
        if os.path.exists(path):
            return True
        time.sleep(1)
    raise FileNotFoundError(f"Arquivo não encontrado após {timeout}s: {path}")
...
if __name__ == "__main__":
    ...
    camada_obj_list = [{
        "arquivo": "C:/Users/souza/Downloads/VideoCreator/assets/videos/intervalo_entre_topicos/clip_0.mp4",
        "inicio": 19.900000000000002,
        "fim": 25.900000000000002,       
        "x": 0,
        "y": 0,
        "largura": VIDEO_WIDTH,
        "altura": VIDEO_HEIGHT
    }, {
        "arquivo": "C:/Users/souza/Downloads/VideoCreator/assets/videos/intervalo_entre_topicos/clip_1.mp4",
        "inicio": 25.900000000000002,
        "fim": 31.900000000000002,               
        "x": 0,
        "y": 0,
        "largura": VIDEO_WIDTH,
        "altura": VIDEO_HEIGHT
    }, {
        "arquivo": "C:/Users/souza/Downloads/VideoCreator/assets/videos/intervalo_entre_topicos/clip_2.mp4",
        "inicio": 31.900000000000002,
        "fim": 37.42,               
        "x": 0,
        "y": 0,
        "largura": VIDEO_WIDTH,
    ...
    video_atual = "C:/Users/souza/Downloads/VideoCreator/video_final_com_mais_topicos.mp4"
    for i, camada in enumerate(camada_obj_list):
        saida = f"C:/Users/souza/Downloads/VideoCreator/assets/videos/videos_temp/a/temp_clips_{i}.mp4"
                
        adicionar_camadas_ao_video_base(
            video_base_path=video_atual,
            camadas=[camada],  # só uma camada
            output_path=saida,
            resolucao=RESOLUCAO_VIDEO,
            fps=VIDEO_FPS,
            crf=18,             # 0 ← qualidade máxima (lossless)
            preset="veryfast"      # "slow" ← compressão mais eficiente
        )
        video_atual = saida