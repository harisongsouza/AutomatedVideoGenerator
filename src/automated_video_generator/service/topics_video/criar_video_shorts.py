...
#CONFIGURACAO
PROCESSED_DIR = 'C:/Users/souza/Videos/VideoCreatorHowTo/assets/imagens/imagens_processadas/shorts'

TEMP_WORK_DIR = 'C:/Users/souza/Videos/VideoCreatorHowTo/assets/videos/intervalo_entre_topicos/temp_shorts' # Pasta para frames e vídeos intermediários

KB_ZOOM_START = 1.0
KB_ZOOM_END_RANGE = (1.15, 1.35)
KB_PAN_AMOUNT = 0.08 # % da dimensão da imagem para o pan

IMAGE_FOLDER = "C:/Users/souza/Videos/VideoCreatorHowTo/assets/imagens/imagens_camadas/imagens_sobreposta_camadas/" # CRIE ESTA PASTA E COLOQUE SUAS IMAGENS AQUI

RESOLUCAO_VIDEO = '1080x1920'
VIDEO_WIDTH = 1080
VIDEO_HEIGHT = 1920
VIDEO_FPS = 30

NARRACAO_AUDIO_FILE = 'C:/Users/souza/Videos/VideoCreatorHowTo/assets/audio/narracao.wav'
MUSICA_BACKGROUND_FILE = 'C:/Users/souza/Videos/VideoCreatorHowTo/assets/audio/musica_fundo.mp3'
AUDIO_FINAL_FILE_PATH = 'C:/Users/souza/Videos/VideoCreatorHowTo/assets/audio/audio_final.mp3'

INTRO_VIDEO_PATH = 'C:/Users/souza/Videos/VideoCreatorHowTo/assets/videos/camadas/video_intro.mp4'
    ...
    try:
        ...
        if duracao_clip is not None and duracao_clip > 0:
            if duracao_original > duracao_clip:
                ...
                os.close(temp_fd)
                arquivo_temporario = temp_path
                input_para_processamento = repetir_ate_duracao(input_path, temp_path, duracao_clip)
                if not input_para_processamento:
                    return None
        elif duracao_clip is not None and duracao_clip <= 0:
        ...
        """ # Adicionar texto com fade
        if inserir_texto:
            texto = inserir_texto[0].get("text", "")
            fonte = inserir_texto[0].get("font", "Arial")
            tamanho = inserir_texto[0].get("size", 70)
            cor = inserir_texto[0].get("color", "white")
            box_color = inserir_texto[0].get("box_color", "black@0.5")
            box_border = inserir_texto[0].get("box_border", 10)
            fadein = inserir_texto[0].get("fadein", False)
...
def recortar_audio(arquivo_entrada: str, inicio: str, duracao: str, arquivo_saida: str):
    """
    Recorta um arquivo de áudio usando FFmpeg.

    Args:
        arquivo_entrada (str): O caminho para o arquivo de áudio de entrada (ex: 'audio.mp3').
        inicio (str): O tempo de início do recorte em segundos (ex: '60.5' para 1 minuto e 0.5 segundos).
        duracao (str): A duração do recorte em segundos (ex: '30').
        arquivo_saida (str): O nome do arquivo de saída (ex: 'audio_recortado.mp3').
    """
    # Constrói o comando FFmpeg
    comando = [
        'ffmpeg',
        '-i', arquivo_entrada,  # Arquivo de entrada
        '-ss', inicio,          # Ponto de início do recorte
        '-t', duracao,          # Duração do recorte
    ...
    try:
        ...
        # stdout e stderr são capturados para não poluir o console principal
        print(f"Executando comando: {' '.join(comando)}")
        subprocess.run(comando, check=True, capture_output=True, text=True)
        print(f"✅ Áudio recortado com sucesso e salvo como '{arquivo_saida}'")
    except FileNotFoundError:
        print("❌ Erro: FFmpeg não encontrado. Verifique se ele está instalado e no PATH do seu sistema.")
        sys.exit(1)
    except subprocess.CalledProcessError as e:
        # Se o FFmpeg retornar um erro, ele será capturado aqui
        print(f"❌ Ocorreu um erro ao executar o FFmpeg:")
        print(f"Comando: {' '.join(e.cmd)}")
        print(f"Stderr: {e.stderr}")
        sys.exit(1)
...
def pegar_tempo_inicio_e_fim_do_audio(listala):
    aaaa = listala[0]["inicio"]
    bbbb = listala[-1]["fim"]
...
def main():
    ...
    # --- RENDER ---
    with open("C:/Users/souza/Videos/VideoCreatorHowTo/data/final_list_render_shorts.json", "w", encoding="utf-8") as f:
        json.dump(clips, f, ensure_ascii=False, indent=2)
              
    ordenar_json_por_inicio("C:/Users/souza/Videos/VideoCreatorHowTo/data/final_list_render_shorts.json", "C:/Users/souza/Videos/VideoCreatorHowTo/data/final_list_render_shorts.json")

    with open("C:/Users/souza/Videos/VideoCreatorHowTo/data/final_list_render_shorts.json", 'r', encoding="utf-8") as f:
        intervalos_final_list = json.load(f)
        
    tempo_inicio_audio, fim = pegar_tempo_inicio_e_fim_do_audio(intervalos_final_list)

    tempo_duracao_audio = fim - tempo_inicio_audio
    recortar_audio(AUDIO_FINAL_FILE_PATH, str(tempo_inicio_audio), str(tempo_duracao_audio), 'meu_audio_recortado.mp3')
                            
    concatenar_clips_com_narracao(intervalos_final_list, "C:/Users/souza/Videos/VideoCreatorHowTo/meu_audio_recortado.mp3", "C:/Users/souza/Videos/VideoCreatorHowTo/end_short_video.mp4")
