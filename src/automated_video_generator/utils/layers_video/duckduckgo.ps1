...
$entrada = $args[0]
Write-Output "Script 1: Parametro: $entrada"
...
Write-Output "⏳ Aguardando o download de urls.txt..."
...
if (Test-Path $arquivoDestino) {
    Write-Output "✅ urls.txt detectado em '$arquivoDestino'."
} else {
    Write-Output "⚠️ Tempo limite atingido e urls.txt não foi encontrado."
}
